...
.session-card{background:var(--surface-2);border:1px solid var(--line);border-radius:8px;padding:14px;display:grid;gap:12px}.session-head{display:flex;justify-content:space-between;gap:12px;align-items:start;flex-wrap:wrap}.session-title{font-size:1.05rem;font-weight:750;margin:0}.session-meta{display:flex;flex-wrap:wrap;gap:8px 14px;color:var(--muted);font-size:.82rem}.session-meta strong{color:var(--text);font-weight:650}.progress-track{height:8px;background:#0b1015;border:1px solid var(--line);border-radius:999px;overflow:hidden}.progress-fill{height:100%;background:linear-gradient(90deg,var(--accent-2),var(--accent));border-radius:999px}.kv-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(160px,1fr));gap:8px}.kv{background:#0b1015;border:1px solid var(--line);border-radius:6px;padding:8px 10px}.kv span{display:block;color:var(--muted);font-size:.68rem;text-transform:uppercase;letter-spacing:.06em}.kv strong{display:block;margin-top:3px;font-size:.82rem;word-break:break-word}.stream-chip{display:inline-flex;align-items:center;gap:6px;border:1px solid var(--line);border-radius:6px;padding:.25rem .5rem;font-size:.72rem;background:#0b1015;margin:2px 4px 2px 0}.stream-chip.selected{border-color:#315e47;color:var(--ok)}details.session-details{border-top:1px solid var(--line);padding-top:10px}details.session-details summary{cursor:pointer;color:var(--muted);font-size:.8rem;user-select:none}details.session-details summary:hover{color:var(--text)}.session-file{font-family:ui-monospace,SFMono-Regular,Consolas,monospace;font-size:.72rem;color:var(--muted);word-break:break-all}
</style>
</head>
<body>
<div class="app">
...
<section class="view" id="view-overview" data-title="Overview">
 ...
 <div class="grid metrics" aria-label="Key system metrics">
  ...
  <article class="card"><div class="metric-label">Now playing</div><div class="metric-value" id="metric-sessions">--</div><div class="muted" id="metric-sessions-sub">Active Plex streams</div></article>
 </div>
 <article class="card" style="margin-bottom:14px"><div class="split"><h3>Currently watching</h3><div class="actions"><span class="muted" id="sessions-freshness">Awaiting Plex sessions</span><button class="secondary" type="button" id="sessions-refresh">Refresh streams</button></div></div><p class="muted" style="margin:0 0 12px">Live Plex playback telemetry: client, codecs, bitrate, progress, and transcode / iGPU hardware path.</p><div id="overview-sessions" class="stack loading">Loading active streams</div></article>
 ...
 <article class="card" style="margin-top:14px"><div class="split"><h3>Monitoring snapshot</h3><a class="muted" href="/api/monitoring/snapshot" target="_blank" rel="noopener">Open /api/monitoring/snapshot</a></div><div id="overview-monitoring" class="stack loading">Loading monitoring snapshot</div><p class="muted" style="margin-top:10px">Compact JSON for Uptime Kuma / external watchers. Discord <code>!cine monitor</code> reads the same payload.</p></article>
...
<script>
'use strict';
const state={status:null,summary:null,operations:null,autonomy:null,preservation:null,acquisitionPlan:null,monitoring:null,sessions:null};
...
function fmtDuration(ms){const total=Math.max(0,Math.round(Number(ms||0)/1000));if(!Number.isFinite(total))return '--';const h=Math.floor(total/3600),m=Math.floor((total%3600)/60),s=total%60;if(h>0)return `${h}:${String(m).padStart(2,'0')}:${String(s).padStart(2,'0')}`;return `${m}:${String(s).padStart(2,'0')}`;}
function sessionDisplayTitle(s){const show=s.grandparent_title||'';const episode=s.title||'Untitled';const season=s.parent_title||'';if(show)return season?`${show} · ${season} · ${episode}`:`${show} · ${episode}`;return s.year?`${episode} (${s.year})`:episode;}
function decisionBadge(mode,decision){const text=String(mode||decision||'unknown').toLowerCase();const label=text.includes('transcode')?'Transcode':text.includes('copy')?'Direct Stream':'Direct Play';const klass=text.includes('transcode')?'warn':text.includes('copy')?'warn':'ok';return `<span class="status ${klass}">${escapeHtml(label)}</span>`;}
function streamChip(stream,kind){if(!stream)return '';const bits=[kind.toUpperCase(),stream.codec,stream.language||stream.language_tag,stream.channels?`${stream.channels}ch`:'',stream.width&&stream.height?`${stream.width}x${stream.height}`:'',stream.bitrate?`${Math.round(stream.bitrate/1000)} kbps`:''].filter(Boolean);return `<span class="stream-chip${stream.selected?' selected':''}">${escapeHtml(bits.join(' · '))}</span>`;}
function kv(label,value){if(value===null||value===undefined||value==='')return '';return `<div class="kv"><span>${escapeHtml(label)}</span><strong>${escapeHtml(value)}</strong></div>`;}
function renderSessionCard(s){const user=s.user||{};const player=s.player||{};const media=s.media||{};const playback=s.playback||{};const transcode=s.transcode||{};const selected=s.selected_streams||{};const streams=s.streams||{};const progress=Number(s.progress_percent);const progressSafe=Number.isFinite(progress)?Math.max(0,Math.min(100,progress)):0;const hw=transcode.hw_active||transcode.hw_decoding||transcode.hw_encoding;const res=[media.width&&media.height?`${media.width}×${media.height}`:'',media.video_resolution?`${media.video_resolution}p`:'',media.video_frame_rate||''].filter(Boolean).join(' · ')||'--';const bitrate=media.bitrate?`${Math.round(Number(media.bitrate)/1000)} Mbps`:'--';const file=media.file?String(media.file):'';const shortFile=file.length>90?`…${file.slice(-90)}`:file;const videoChips=(streams.video||[]).map(st=>streamChip(st,'video')).join('');const audioChips=(streams.audio||[]).map(st=>streamChip(st,'audio')).join('');const subChips=(streams.subtitle||[]).map(st=>streamChip(st,'sub')).join('');const title=sessionDisplayTitle(s);return `<article class="session-card"><div class="session-head"><div><h4 class="session-title">${escapeHtml(title)}</h4><div class="session-meta" style="margin-top:6px"><span>${badge(s.type||'media')}</span><span>${badge(player.state||s.state||'playing')}</span>${decisionBadge(playback.mode,playback.video_decision)}${hw?`<span class="status ok">HW ${escapeHtml(transcode.hw_decoding||transcode.hw_encoding||'iGPU')}</span>`:''}${s.library_section?`<span class="muted">${escapeHtml(s.library_section)}</span>`:''}${s.content_rating?`<span class="muted">${escapeHtml(s.content_rating)}</span>`:''}</div></div><div class="muted" style="text-align:right;font-size:.78rem">Session ${escapeHtml(s.session_key||'--')}<br>Key ${escapeHtml(s.rating_key||'--')}</div></div><div class="progress-track" aria-hidden="true"><div class="progress-fill" style="width:${progressSafe}%"></div></div><div class="session-meta"><span><strong>${escapeHtml(fmtDuration(s.view_offset_ms))}</strong> / ${escapeHtml(fmtDuration(s.duration_ms))}</span><span>${escapeHtml(Number.isFinite(progress)?`${progress.toFixed(1)}%`:'--')}</span><span>${escapeHtml(s.remaining_ms!=null?`${fmtDuration(s.remaining_ms)} left`:'')}</span></div><div class="session-meta"><span>User <strong>${escapeHtml(user.title||s.user_name||'Unknown')}</strong></span><span>Player <strong>${escapeHtml(player.product||player.title||s.player_name||'Unknown')}</strong></span><span>Platform <strong>${escapeHtml(player.platform||'--')}</strong></span><span>Device <strong>${escapeHtml(player.device||'--')}</strong></span><span>IP <strong>${escapeHtml(player.address||'--')}</strong></span><span>Local <strong>${escapeHtml(player.local==null?'--':player.local?'yes':'no')}</strong></span><span>Secure <strong>${escapeHtml(player.secure==null?'--':player.secure?'yes':'no')}</strong></span><span>Relayed <strong>${escapeHtml(player.relayed==null?'--':player.relayed?'yes':'no')}</strong></span></div><div class="kv-grid">${kv('Container',media.container)}${kv('Video', [media.video_codec,media.video_profile].filter(Boolean).join(' / '))}${kv('Audio',[media.audio_codec,media.audio_channels?`${media.audio_channels}ch`:''].filter(Boolean).join(' · '))}${kv('Resolution',res)}${kv('Bitrate',bitrate)}${kv('Size',media.size!=null?fmtBytes(media.size):'--')}${kv('Video path',playback.video_decision||'--')}${kv('Audio path',playback.audio_decision||'--')}${kv('Subtitles',playback.subtitle_decision||(selected.subtitle?'burn/soft':'none'))}${kv('Client version',player.version||'--')}</div>${transcode.active?`<div class="kv-grid">${kv('Transcode speed',transcode.speed!=null?`${Number(transcode.speed).toFixed(2)}x`:'--')}${kv('Transcode progress',transcode.progress!=null?`${Number(transcode.progress).toFixed(1)}%`:'--')}${kv('Throttled',transcode.throttled==null?'--':transcode.throttled?'yes':'no')}${kv('HW decode',transcode.hw_decoding_title||transcode.hw_decoding||'off')}${kv('HW encode',transcode.hw_encoding_title||transcode.hw_encoding||'off')}${kv('Output',[transcode.container,transcode.video_codec,transcode.audio_codec,transcode.width&&transcode.height?`${transcode.width}x${transcode.height}`:''].filter(Boolean).join(' · ')||'--')}</div>`:''}${file?`<div class="session-file" title="${escapeHtml(file)}">${escapeHtml(shortFile)}</div>`:''}<details class="session-details"><summary>All media streams (${escapeHtml((streams.video||[]).length+(streams.audio||[]).length+(streams.subtitle||[]).length)} tracks)</summary><div style="margin-top:8px">${videoChips||'<span class="muted">No video streams</span>'}${audioChips}${subChips}</div><div class="kv-grid" style="margin-top:10px">${kv('Selected video',selected.video?[selected.video.codec,selected.video.width&&selected.video.height?`${selected.video.width}x${selected.video.height}`:'',selected.video.bit_depth?`${selected.video.bit_depth}-bit`:''].filter(Boolean).join(' · '):'--')}${kv('Selected audio',selected.audio?[selected.audio.codec,selected.audio.channels?`${selected.audio.channels}ch`:'',selected.audio.language||selected.audio.language_tag||''].filter(Boolean).join(' · '):'--')}${kv('Selected subtitle',selected.subtitle?[selected.subtitle.codec,selected.subtitle.language||selected.subtitle.language_tag||'',selected.subtitle.title||''].filter(Boolean).join(' · '):'None')}</div></details></article>`;}
function renderSessions(){const data=state.sessions||{};const sessions=data.sessions||[];const el=document.getElementById('overview-sessions');const countEl=document.getElementById('metric-sessions');const subEl=document.getElementById('metric-sessions-sub');const fresh=document.getElementById('sessions-freshness');countEl.textContent=fmtNumber(sessions.length);subEl.textContent=sessions.length===1?'Active Plex stream':'Active Plex streams';fresh.textContent=data.generated_at?`Updated ${relativeTime(data.generated_at)}`:'Updated just now';el.className='stack';el.innerHTML=sessions.length?sessions.map(renderSessionCard).join(''):empty('No active Plex streams. iGPU / transcoder idle.');}
let sessionsTimer=null;
async function loadSessions(force=false){const el=document.getElementById('overview-sessions');try{if(force||!state.sessions){if(!state.sessions){el.className='stack loading';el.textContent='Loading active streams';}state.sessions=await apiFetch('/api/sessions',{signal:AbortSignal.timeout(8000)});}renderSessions();}catch(error){el.className='stack';el.innerHTML=errorState(error);document.getElementById('metric-sessions').textContent='--';document.getElementById('sessions-freshness').textContent='Sessions unavailable';}}
function ensureSessionsPolling(active){if(sessionsTimer){clearInterval(sessionsTimer);sessionsTimer=null;}if(active){sessionsTimer=setInterval(()=>{const overview=document.getElementById('view-overview');if(overview&&!overview.hidden)loadSessions(true);},5000);}}
function setView(name){document.querySelectorAll('.view').forEach(view=>view.hidden=view.id!==`view-${name}`);document.querySelectorAll('[data-view]').forEach(button=>button.setAttribute('aria-current',button.dataset.view===name?'page':'false'));const view=document.getElementById(`view-${name}`);document.getElementById('page-title').textContent=view?.dataset.title||'CineSwarm';location.hash=name==='overview'?'':name;closeMenu();loadView(name);ensureSessionsPolling(name==='overview');}
...
const mon=state.monitoring||{};const monDownloads=mon.downloads||{};const monWorker=mon.worker||{};const monFailures=mon.recent_failures||[];const lastAlert=monFailures[0];const pressure=mon.queue_pressure||{};const integrity=mon.library_integrity||{};const integrityMovies=integrity.movies||{};const integritySeries=integrity.series||{};const pressureSources=pressure.sources||{};const obs=mon.acquisition_observations||{};document.getElementById('overview-monitoring').className='stack';document.getElementById('overview-monitoring').innerHTML=`<div class="service-row"><div><strong>Overall status</strong><div class="muted">Generated ${escapeHtml(relativeTime(mon.generated_at))}</div></div>${badge(mon.overall_status||'unknown')}</div><div class="service-row"><div><strong>Queue truth</strong><div class="muted">Arr movies · series · SAB (active/limit ${escapeHtml(pressure.active??monDownloads.total??0)}/${escapeHtml(pressure.limit??'--')})</div></div><strong>${fmtNumber((pressureSources.radarr??monDownloads.movies)??0)} · ${fmtNumber((pressureSources.sonarr??monDownloads.series)??0)} · ${fmtNumber(pressureSources.sabnzbd??0)}</strong></div><div class="service-row"><div><strong>Queue pressure</strong><div class="muted">${escapeHtml((pressure.sample||[]).slice(0,2).map(s=>s.title||s.status||'').filter(Boolean).join(' · ')||'No active downloads')}</div></div>${badge(pressure.pressured?'pressured':(pressure.status||'clear'))}</div><div class="service-row"><div><strong>Library integrity</strong><div class="muted">Movies without file · on disk not in Plex · series path missing</div></div><strong>${fmtNumber(integrityMovies.without_file)} · ${fmtNumber(integrityMovies.file_not_indexed)} · ${fmtNumber(integritySeries.path_missing)}</strong></div><div class="service-row"><div><strong>Acquisition observations</strong><div class="muted">${escapeHtml(Object.entries(obs.states||{}).map(([k,v])=>`${k}:${v}`).join(' · ')||'No recent observations')}</div></div><strong>${fmtNumber(obs.recent_count||0)}</strong></div><div class="service-row"><div><strong>Now playing</strong><div class="muted">Active Plex sessions</div></div><strong>${fmtNumber(mon.active_sessions||0)}</strong></div><div class="service-row"><div><strong>Pending approvals</strong><div class="muted">Tasks awaiting operator action</div></div><strong>${fmtNumber(mon.pending_approvals||0)}</strong></div><div class="service-row"><div><strong>Worker</strong><div class="muted">${escapeHtml(monWorker.healthy?'Heartbeat current':'Heartbeat stale or unknown')}</div></div>${badge(monWorker.healthy?'healthy':'stale')}</div><div class="service-row"><div><strong>Emergency stop</strong></div>${badge(mon.emergency_stop?'active':'disabled')}</div><div class="service-row"><div><strong>Last alert</strong><div class="muted">${escapeHtml(lastAlert?(String(lastAlert.type||'issue')+' · '+(lastAlert.action||lastAlert.error||lastAlert.status||'review')):'No recent failures in snapshot')}</div></div>${badge(lastAlert?(lastAlert.status||'attention'):'clear')}</div>`;}
async function loadOverview(force=false){const fail=(id,error)=>{const node=document.getElementById(id);if(!node)return;node.className='stack';node.innerHTML=errorState(error);};const timed=(url)=>apiFetch(url,{signal:AbortSignal.timeout(8000)});try{if(force||!state.status){const settled=await Promise.allSettled([timed('/api/status'),timed('/api/operational-summary?hours=24'),timed('/api/operations/queue?limit=20'),timed('/api/transcode-shield'),timed('/api/monitoring/snapshot')]);const value=(result)=>result.status==='fulfilled'?result.value:null;const status=value(settled[0]),summary=value(settled[1]),operations=value(settled[2]),shield=value(settled[3]),monitoring=value(settled[4]);if(status)state.status=status;else if(!state.status)state.status={};if(summary)state.summary=summary;else if(!state.summary)state.summary={};if(operations)state.operations=operations;else if(!state.operations)state.operations={};if(shield)state.shield=shield;else if(!state.shield)state.shield={};state.monitoring=monitoring||state.monitoring||null;if(!status&&!summary&&!operations&&!shield&&!monitoring){const firstReject=settled.find(result=>result.status==='rejected');throw firstReject?firstReject.reason:new Error('Overview APIs unavailable');}if(!monitoring){const monReject=settled[4];if(monReject&&monReject.status==='rejected')fail('overview-monitoring',monReject.reason||new Error('Monitoring snapshot unavailable'));}}try{renderOverview();}catch(error){fail('overview-services',error);fail('overview-issues',error);if(document.getElementById('overview-monitoring')?.classList.contains('loading'))fail('overview-monitoring',error);}}catch(error){['overview-services','overview-issues','overview-monitoring'].forEach(id=>fail(id,error));}await Promise.allSettled([loadSessions(force),(async()=>{try{const result=await timed('/api/decisions');const decisions=result.decisions||[];const el=document.getElementById('overview-decisions');el.className='stack';el.innerHTML=decisions.length?decisions.slice(0,8).map(d=>`<div class="decision"><div class="split"><strong>${escapeHtml(d.subject||d.category||'Decision')}</strong>${badge(d.decision)}</div><div class="muted">${escapeHtml(d.category||'action')} · ${escapeHtml(relativeTime(d.created_at))}</div><div>${escapeHtml(typeof d.reasons==='string'?d.reasons:JSON.stringify(d.reasons||{}))}</div><div class="actions"><button class="secondary" type="button" data-decision-feedback="good" data-decision-id="${escapeHtml(d.decision_id)}">Mark useful</button><button class="secondary" type="button" data-decision-feedback="bad" data-decision-id="${escapeHtml(d.decision_id)}">Mark unhelpful</button></div></div>`).join(''):empty('No decisions have been recorded.');}catch(error){fail('overview-decisions',error);}})()]);}
